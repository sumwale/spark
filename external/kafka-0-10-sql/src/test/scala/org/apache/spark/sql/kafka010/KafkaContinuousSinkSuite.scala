...
class KafkaContinuousSinkSuite extends KafkaContinuousTest {
  ...
  test("streaming - write to non-existing topic") {
    ...
    while (cause.getCause ne null) {
      cause = cause.getCause
    }
    val message = cause.getMessage.toLowerCase(Locale.ROOT)
    assert(message.contains("job aborted") || message.contains("failed to update metadata"))
  }
