...
// scalastyle:on
  ...
  protected def getRootDir(projectRelativeDir: String): String =
    SparkFunSuite.getRootDir(projectRelativeDir)
}

object SparkFunSuite {
  def getRootDir(projectRelativeDir: String): String = {
    sys.props.get("spark.project.home") match {
      case Some(h) => h
      case None => s"${sys.props.get("user.dir")}/$projectRelativeDir"
    }
  }
}
