...
private[ui] class SnappyStructuredStreamingPage(parent: SnappyStreamingTab)
  ...
  def commonHeaderNodesSnappy(request: HttpServletRequest): Seq[Node] = {
    <link rel="stylesheet" type="text/css"
          href={UIUtils.prependBaseUri(request, "/static/snappydata/snappy-streaming.css")}/>
    <script src={UIUtils.prependBaseUri(request, "/static/snappydata/d3.js")}></script>
    <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script src={UIUtils.prependBaseUri(request,
      "/static/snappydata/jquery.sparkline.min.js")}></script>
    <script src={UIUtils.prependBaseUri(request,
      "/static/snappydata/snappy-commons.js")}></script>
    <script src={UIUtils.prependBaseUri(request,
      "/static/snappydata/snappy-streaming.js")}></script>
  }
  ...
  override def render(request: HttpServletRequest): Seq[Node] = {
    ...
    val pageContent = commonHeaderNodesSnappy(request) ++ createMainContent

    UIUtils.headerSparkPage(request, pageHeaderText, pageContent, parent,
      Some(500), useDataTables = true)
  }
