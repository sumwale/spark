      ...
      withSQLConf(s"spark.datasource.$readSource.$readOptionName" -> "true") {
        testPositiveCaseWithQuery(readSource, writeSource, trigger) { _ =>
          eventually(timeout(streamingTimeout)) {
            ...
            assert((LastWriteOptions.options eq null) ||
                LastWriteOptions.options.getBoolean(readOptionName, false) == false)
      ...
      withSQLConf(s"spark.datasource.$writeSource.$writeOptionName" -> "true") {
        testPositiveCaseWithQuery(readSource, writeSource, trigger) { _ =>
          eventually(timeout(streamingTimeout)) {
            ...
            assert((LastReadOptions.options eq null) ||
                LastReadOptions.options.getBoolean(writeOptionName, false) == false)
